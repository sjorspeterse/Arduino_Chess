...
byte decoder(byte k)
{
	...
	if(dataType == 2) {
		...
		type = bvSerial.intData; // type can be 1 (pawn), 2 (knight), 3 (bishop), 4 (rook), 5 (queen), 6 (king), 7 (Control Z), 8 (castling kingside), 9 (castling queenside), 10 (super user), 11 (calibrate) or 12 (wondering)
				...
				case 1: //second character (first coordinate)
					if (in > 48 && in < 59) {
						...
						if (flow == 3) {
							...
							flow = 1;
							 return 1; // break out of the decoder() function
...
byte a(byte b,char d[]) // b is the number of the function,
{
	...
	for(byte i = 1; d[i] != 127; i++) {
		if (i != 1) bvSerial.sendToBV(",");
		bvSerial.sendToBV(d[i],10);
	}
	bvSerial.sendToBV(")");
	g++;	
	*/
...
byte possible(byte k)   
{
	...
	if (CB[x2][y2] > 0 + 6 * turn && CB[x2][y2] < 7 + 6 * turn)  {
		feedbacklight(17,2); // flash red light
		 return 0; // don't allow zet() to run
...
byte print(byte k, boolean d, char b, char c) // d can be true (regular move) or false (wondering)
{
	...
	String piece; byte stuk; String space = " ";
	...
	if (d) {
		if(turn) piece = "Black: " + piece + " to " + char((11-c)+62) + char(b+48);
		else piece = "White: " + piece + " to " + char((11-c)+62) + char(b+48);
		bvSerial.sendToBV(piece);
	}
	else {
		if (stuk == 0) piece = "There is nothing" + space + "at " + char((11-c)+62) + char(b+48);
		else {
                    if(stuk > 6) piece = "There is a black " + piece + " at " + char((11-c)+62) + char(b+48);
    	            else piece = "There is a white " + piece + " at " + char((11-c)+62) + char(b+48);
    		}
    		bvSerial.sendToBV(piece);
	}
